    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        for (IntentWrapper intentWrapper : sIntentWrapperList) {
            ...
            switch (intentWrapper.mType) {
                case DOZE:
                    if (Build.VERSION.SDK_INT > Build.VERSION_CODES.LOLLIPOP_MR1) {
                        ...
                        if (pm.isIgnoringBatteryOptimizations(MainApplication.getContext().getPackageName()))
                            break;
                        nothingMatches = false;
                        title = "设置 " + getApplicationName() + " 的电池优化";
                        message = "服务的持续运行需要将 " + getApplicationName() + " 加入到电池优化的忽略名单。\n\n" +
                                "请点击『" + textButton + "』，" +
                ...
                case HUAWEI:
                    nothingMatches = false;
                    title = "设置 " + getApplicationName() + " 的自动启动";
                ...
                case HUAWEI_GOD:
                    nothingMatches = false;
                    title = "加入" + getApplicationName() + " 到受保护的应用名单中";
                ...
                case XIAOMI_GOD:
                    nothingMatches = false;
                    title = "设置 " + getApplicationName() + " 的神隐模式";
                    message = "服务的持续运行需要设置 " + getApplicationName() + " 的神隐模式。\n\n" +
                            "请点击『" + textButton + "』，" +
                ...
                case MEIZU_GOD:
                    nothingMatches = false;
                    title = "设置 " + getApplicationName() + " 的耗电管理";
                ...
                case OPPO:
                    nothingMatches = false;
                    title = "设置 " + getApplicationName() + " 的自动启动";
                    message = "服务的持续运行需要将 " + getApplicationName() + " 加入到自启动白名单。\n\n" +
                            "请点击『" + textButton + "』，" +
                            "在弹出的『自启动管理』中，将 " + getApplicationName() + " 对应的开关打开。";
                    break;
                case OPPO_GOD:
                    nothingMatches = false;
                    title = "设置 " + getApplicationName() + " 的后台运行";
                ...
                case LETV_GOD:
                    nothingMatches = false;
                    title = "设置 " + getApplicationName() + " 的应用保护";
                    message = "服务的持续运行需要防止 " + getApplicationName() + " 被自动清理。\n\n" +
                            "请点击『" + textButton + "』，" +
                            "在弹出的『应用保护』中，将 " + getApplicationName() + " 对应的开关关闭。";
                    break;
                case LENOVO:
                    nothingMatches = false;
                    title = "设置 " + getApplicationName() + " 的后台 GPS 和后台运行。";
            ...
            if (!nothingMatches) {
                ...
                backgroundPermissionSlide.setTitle("第 " + count + " 步\n" + title);
        ...
        if (MainApplication.isUsageStatsExists()) {
            count++;
            UsagePermissionSlide usagePermissionSlide = new UsagePermissionSlide();
            usagePermissionSlide.setCount(count);
            addSlide(usagePermissionSlide);
        }
    }
}
